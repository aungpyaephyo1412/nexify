...
const SideNavigation = () => {
  ...
  return (
    <section className="col-span-2 sticky top-0 h-screen border-e border-e-gray-500/50 hidden lg:block">
      <Link href={"/"} className="flex gap-2 items-center p-6">
        <Logo />
      </Link>
      <ScrollArea className="w-full h-[calc(100vh-100px)] px-3">
        <div className="py-2 space-y-3">
          {sideNavigationData.map((tag) => (
            <Link
              href={tag.href}
              className={cn(
                "flex gap-3 items-center hover:bg-black/10 py-3 px-5 rounded-xl",
